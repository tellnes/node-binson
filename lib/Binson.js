  ...
  case 'object':
    ...
    // Error
      ...
      if (custom) {
        ...
        size += LENGTH_BOOLEAN; // deep
        if (custom.calculate) {
          size = custom.calculate(object, size);
        } else {
...
Binson.prototype.encodeElement = function(bw, object) {
  ...
  case 'number':
    ...
    if (isNaN(object)) {
      ...
      bw.write(BOOLEAN_FALSE, LENGTH_BOOLEAN); // sign pos


    } else if (object == -Infinity) {
      bw.write(NUMBER_INFINITY, LENGTH_NUMBER);
      bw.write(BOOLEAN_TRUE, LENGTH_BOOLEAN); // sign neg


    } else if (object === (object | 0)) { // interger
      bw.write(NUMBER_INTEGER, LENGTH_NUMBER);

      var sign = BOOLEAN_FALSE; // pos
      if (object < 0) {
        sign = BOOLEAN_TRUE; // neg
        object = -object;
      }
  ...
  case 'object':
    ...
    // Binary
      ...
      bw.write(TYPE_BINARY, LENGTH_TYPE);
    ...
    // Error
      ...
      if (custom) {
        ...
        bw.write(custom.deep ? BOOLEAN_TRUE : BOOLEAN_FALSE, LENGTH_BOOLEAN);

        this.encodeElement(bw, custom.encode(object, bw));
...
Binson.prototype.decodeElement = function(br, type) {
  ...
  case TYPE_NUMBER:
    ...
    case NUMBER_INFINITY:
      return br.read(LENGTH_BOOLEAN) === BOOLEAN_TRUE ? -Infinity : Infinity;
    ...
    case NUMBER_INTEGER:
      ...
      if (br.read(LENGTH_BOOLEAN) === BOOLEAN_TRUE) {
        value = -value;
      }
  ...
  case TYPE_SPECIAL:
    ...
    case SPECIAL_CUSTOM:
      ...
      var deep = br.read(LENGTH_BOOLEAN) === BOOLEAN_TRUE;

      var value = this.decodeElement(br, br.read(LENGTH_TYPE));

      value = custom.decode( value
                           , deep ? this.decodeElement(br, TYPE_PLAIN) : null
                           , br
                           );
