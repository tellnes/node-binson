...
Binary.prototype.register = function(index, info) {
  this.customTypes[index] = info;
};

Binary.prototype.calculate = function(obj) {
  var size = this.calculateElement(obj, 0);
  return Math.ceil(size / 8);
};

Binary.prototype.encode = function (object, buffer, offset) {
  if (!buffer) {
...
Binary.prototype.decode = function(buffer, offset) {
...
Binary.prototype.calculateElement = function (object, size) {
  size = size || 0;
  ...
  case 'object':
    ...
    // Error
      ...
      if (custom) {
        ...
        size += LENGTH_SPECIAL;
        size += calcIntegerBits(this.customTypes.length);
...
Binary.prototype.encodeElement = function(bw, object) {
  ...
  case 'object':
    ...
    // Error
      ...
      if (custom) {
        ...
        bw.write(this.customTypes.length, calcIntegerBits(this.customTypes.length));
...
Binary.prototype.decodeElement = function(br, type) {
  ...
  case TYPE_SPECIAL:
    ...
    case SPECIAL_CUSTOM:
      ...
      custom = br.read(calcIntegerBits(this.customTypes.length));
...
exports = module.exports = new Binary();
